...
function partition(arr, callback) {
    return arr.reduce((paritioned, el) => {
 ...
 */
export async function* findTestModules(includeModules, excludeModules, root = cwd()) {
    ...
    async function* expandDirectory(d) {
    ...
    const excludeUrlPatterns = excludeUrls.map((url) => RegExp(url));
    const shouldIncludeUrl = (url) => !excludeUrlPatterns.some((p) => !!url.match(p));
 ...
 * @param testModules
 */
function renderTestFile(testModules) {
    let testFile = "";
...
export async function runTestModules({ include = ["."], exclude = [], allowNone = false, exitOnFail = false, only = /[^\s]/, skip = /^\s*$/, disableLog = false } = {}) {
    let moduleCount = 0;
    const testModules = [];
...
async function main() {
    ...
    const include = parsedArgs._.length > 0
        ? parsedArgs._.flatMap((fileGlob) => fileGlob.split(","))
        : ["."];
    const exclude = parsedArgs.exclude != null ? parsedArgs.exclude.split(",") : [];
