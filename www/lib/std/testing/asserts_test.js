...
import { assert, assertNotEquals, assertStrContains, assertArrayContains, assertMatch, assertEquals, assertThrows, AssertionError, equal, fail, unimplemented, unreachable } from "./asserts.js";
import { red, green, white, gray, bold } from "../fmt/colors.js";
const { test } = Deno;
...
const added = (s) => green(bold(s));
const removed = (s) => red(bold(s));
test({
    name: "pass case",
    fn() {
...
test({
    ...
    fn() {
        assertThrows(() => assertEquals(1, 2), AssertionError, [...createHeader(), removed(`-   1`), added(`+   2`), ""].join("\n"));
    }
});
test({
    name: "failed with number vs string",
    fn() {
        assertThrows(() => assertEquals(1, "1"), AssertionError, [...createHeader(), removed(`-   1`), added(`+   "1"`)].join("\n"));
    }
});
test({
