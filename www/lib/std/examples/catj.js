...
function printObject(obj, path) {
    for (var _i = 0, _a = Object.keys(obj); _i < _a.length; _i++) {
        var key = _a[_i];
        var value = obj[key];
        var nodePath = path + colors.cyan(".") + key;
...
function printArray(array, path) {
    for (var index in array) {
        ...
        var nodePath = (path ? path : colors.cyan(".")) + "[" + colors.cyan(index) + "]";
        if (Array.isArray(value)) {
            printArray(value, nodePath);
        }
        else if (isObject(value)) {
...
if (parsedArgs._[0] === "-") {
    var contents = await Deno.readAll(Deno.stdin);
    var json = JSON.parse(decoder.decode(contents));
    print(json);
}
else {
    for (var _i = 0, _a = parsedArgs._; _i < _a.length; _i++) {
        var fileName = _a[_i];
        var fileContents = await Deno.readFile(fileName);
        var json = JSON.parse(decoder.decode(fileContents));
        print(json);
    }
}
//# sourceMappingURL=catj.js.map
